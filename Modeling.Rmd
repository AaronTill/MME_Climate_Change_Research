...
ridge_regression_lambda_summer <- cv.glmnet(x_ridge, y_ridge, family = "binomial", alpha = 0, lambda = c(10^-15, 10^-12, 10^-9)) # alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
...
lasso_regression_coords_lambda_summer <- cv.glmnet(x_all, y_all, family = "binomial", alpha = 1, lambda = c(10^-15, 10^-12, 10^-9)) # alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
...
lasso_coords_regression_test <- model.matrix(Summerkill ~ . ,train_modeling_data)
...
```{r}
reg_train <- glm(Summerkill ~  Mean_Surf_Temp + Mean_Surf_Zscore + layer_dif+ Schmidt + population + V1 + V2 , family = 'binomial', data=train_data) 
``` 
