...
train_indices <- sample(1:nrow(main_data_census), size = 330212, replace = FALSE) #half
...
x_summer <- model.matrix(Summerkill ~ Mean_Surf_Temp + Mean_Surf_Zscore + layer_dif + Schmidt + Cumulative_Above_0 + population, test_data) 


y_summer <- ifelse(test_data$Summerkill == 1, 1, 0)
...
```
# Ridge regression Summerkill final

x_summer 
y_summer
from lasso regression above 

```{r}

lambdatest <- glmnet(x_summer, y_summer, family = "binomial", alpha = 0)
plot(lambdatest, xvar = 'lambda')
...
ridge_regression_lambda_summer <- cv.glmnet(x_summer, y_summer, family = "binomial", alpha = 0) # alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
...
ridge_regression_model_summer <- glmnet(x_summer, y_summer, family = "binomial", alpha = 0, lambda = ridge_regression_lambda_summer$lambda.min) 
# alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
#use $lambda.1se instead of $lambda.min for simpler but less accurate model
...
# Lasso + Coordinates Final

```{r source - }

# TEST !!

x_summer <- model.matrix(Summerkill ~ Mean_Surf_Temp + Mean_Surf_Zscore + layer_dif + Schmidt + Cumulative_Above_0 + population + V1 + V2, test_data) 
...
lasso_regression_coords_lambda_summer <- cv.glmnet(x_summer, y_summer, family = "binomial", alpha = 1, lambda = grid) # alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
...
lasso_regression_coords_model_summer <- glmnet(x_summer, y_summer, family = "binomial", alpha = 1, lambda = lasso_regression_coords_lambda_summer$lambda.min) 
# alpha = 0 ridge, 1 for lasso, between 0 and 1 for elastic 
#use $lambda.1se instead of $lambda.min for simpler but less accurate model
...
#Building lasso predictions dataset 
...
newx<- model.matrix(Summerkill ~ Mean_Surf_Temp + Mean_Surf_Zscore + layer_dif  + Schmidt + Cumulative_Above_0 + population, ridge_summer_predictions) #why NEWX has to be a fake model and not just a dataset is unclear)
...
#Building lasso coords predictions dataset 
...
lasso_coords_summer_predictions$Prob <- as.vector(predict(lasso_regression_model_summer, newx= newx, type = 'response'))


lasso_coords_summer_predictions$quantile <- quantile(lasso_summer_predictions$Prob,probs = c(.01, 25, 75)/100, na.rm = TRUE)
