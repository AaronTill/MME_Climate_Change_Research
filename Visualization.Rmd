...
boxplot2 <- mean_surf(combined_data_restricted) %>%
  filter(Month == 'Jul' | Month == 'Jun' | Month == 'Aug' | Month == 'Sep') %>%
  filter(Cause.Category.4 == 'SUMMERKILL' | Cause.Category.4 == 'INFECTIOUS AGENT') %>%   
...
ttest_data_infectious<- filter(mean_surf(combined_data_restricted), Cause.Category.4 == 'INFECTIOUS AGENT')
...
ttest_data_winter_all<- filter(mean_surf(combined_data_restricted), Cause.Category.4 == 'WINTERKILL')
...
xbox <- grid.arrange(boxplot1,boxplot1_winter,  ncol = 2, widths = c(1, 0.7))
ybox <- grid.arrange(boxplot2,ultimate_zscore_boxplot,  boxplot2_winter, ncol = 3, widths = c(1,1, 0.6 ))
...
Wisconsin_map <- get_map(bbox, zoom = 7, maptype = 'watercolor', source = 'stamen')
ggmap(Wisconsin_map)
