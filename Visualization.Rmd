...
leveneTest(y = zscore_temp, group = Cause.Category.4, data = anova_data)
...
tuk
```
```{r Boxplot Zscore Anova winter}

leveneTest(y =temperature ~ Cause.Category.4, data = anova_data_winter, center = mean)

anova_zscore_cause_Type <- aov(zscore_peak_temp ~ Cause.Category.4,anova_data_winter)
summary(anova_zscore_cause_Type)
tuk <- TukeyHSD(anova_zscore_cause_Type)
tuk
...
grid.arrange(logreg_at_risk_lakes_of_wisconsin_period_2, logreg_at_risk_lakes_of_wisconsin_period_3, color_seperated_lakes_of_wisconsin, ncol = 2, padding = 0.1)
