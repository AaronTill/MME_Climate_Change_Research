...
plot2<-ridge_summer_predictions %>%
...
plot3<- ridge_summer_predictions %>%
  filter(Year >2060) %>%
  group_by(Year) %>%
  summarise(Summerkill = sum(Summerkill_forecast), Temp = mean(Mean_Surf_Temp) ) %>%
ggplot(aes(x=factor(Year),y = Summerkill, fill = Temp)) +
...
summarized_lasso_summer_predictions_p2 <- ridge_summer_predictions %>%
  filter(Year > 2030 & Year < 2070) %>%
  group_by(WBIC) %>%
  summarise(Prob = sum(Prob), V1 = mean(V1), V2 = mean(V2)) 

summarized_lasso_summer_predictions_p3 <- ridge_summer_predictions %>%
  filter(Year > 2070) %>%
  group_by(WBIC) %>%
  summarise(Prob = sum(Prob), V1 = mean(V1), V2 = mean(V2)) 
